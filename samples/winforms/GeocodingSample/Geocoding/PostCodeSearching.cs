...
namespace Geocoding
{
    public partial class PostCodeSearching : UserControl
    {
        public PostCodeSearching()
        {
            InitializeComponent();
            cboSourceText.Text = cboSourceText.Items[0].ToString();
        }

        private void PostCodeSearching_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string dataPath = Path.Combine(SampleHelper.GetDataPath("PostCode"));
            Geocoder geocoder = new Geocoder(dataPath);
            geocoder.MatchingPlugIns.Add(new PostcodeDbfMatchingPlugin(dataPath));
        ...
        private void PopulateAddressResultList(Collection<GeocoderMatch> matchResult)
        {
            ...
            {
                if (matchItem.NameValuePairs.ContainsKey("PostCode"))
                {
                    lstResult.Items.Add(new ListItem(matchItem, new string[] { "PostCode", "PlaceName", "Admin Level 1" }));
                }
            }
